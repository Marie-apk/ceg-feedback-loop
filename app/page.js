...
const SEGMENTS = ["Commercial", "Enterprise", "Strategic", "Federal", "Other"];
const PRIORITIES = [
...
const SAMPLE_ENTRIES = [
  ...
  { id: 2, date: "2026-03-01", submittedBy: "João Corona", customer: "ZUP", segment: "Enterprise", theme: "feature", product: "Employee Center Pro", priority: "medium", request: "Better dashboard customization options needed", frequency: "first" },
  ...
  { id: 6, date: "2026-02-25", submittedBy: "João Corona", customer: "ZUP", segment: "Enterprise", theme: "training", product: "CSM", priority: "low", request: "Request for advanced CSM workshop for power users", frequency: "first" },
...
export default function FeedbackApp() {
  ...
  const [formData, setFormData] = useState({ theme: "", product: "", customer: "", segment: "", priority: "", request: "", frequency: "" });
        ...
        {view === "dashboard" && (
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 24 }}>
              {[
                { label: "Total Feedback", value: entries.length, icon: <MessageCircle size={18} />, color: "#6366F1", sub: "this month" },
                { label: "High Priority", value: highPriority, icon: <AlertTriangle size={18} />, color: "#DC2626", sub: `${Math.round((highPriority / entries.length) * 100)}% of total` },
                ...
                {formStep === 1 && (
                  <div>
                    ...
                    <div style={{ marginBottom: 20 }}>
                      ...
                      <div style={{ display: "flex", gap: 8 }}>
                        {SEGMENTS.map((s) => (
                          <button
                            key={s}
                            onClick={() => setFormData({ ...formData, segment: s.toLowerCase() })}
                            style={{
                              padding: "8px 16px", borderRadius: 8, border: `1px solid ${formData.segment === s.toLowerCase() ? "#6366F1" : "rgba(255,255,255,0.08)"}`,
                              background: formData.segment === s.toLowerCase() ? "rgba(99,102,241,0.15)" : "rgba(255,255,255,0.02)",
                              color: formData.segment === s.toLowerCase() ? "#A5B4FC" : "#94A3B8", fontSize: 13, fontWeight: 500, cursor: "pointer", transition: "all 0.2s",
                            }}
                          >
                            {s}
                          </button>
                    ...
                    <div style={{ display: "flex", gap: 10 }}>
                      ...
                      <button
                        onClick={() => formData.product && formData.customer && setFormStep(2)}
                        style={{ flex: 1, padding: "12px 20px", borderRadius: 10, border: "none", background: formData.product && formData.customer ? "linear-gradient(135deg, #6366F1, #7C3AED)" : "rgba(255,255,255,0.05)", color: formData.product && formData.customer ? "white" : "#64748B", fontSize: 14, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}
                      >
